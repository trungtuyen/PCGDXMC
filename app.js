(function(){
  ...
  file.addEventListener('change',()=>{
    ...
    setStatus(selectedFiles.length===1?`Đã chọn: ${selectedFiles[0].name}. Nhấn “Phân tích và tạo biểu”.`:selectedFiles.length>1?`Đã chọn ${selectedFiles.length} file điều tra. Phần mềm sẽ gộp dữ liệu.`:'Chọn file điều tra Excel để bắt đầu.','info');
  });
  analyze.addEventListener('click',run);
  exportBtn.addEventListener('click',()=>{if(result)PCGDEngine.exportResult(result,sourceBase())});
  document.querySelectorAll('[data-export-group]').forEach(btn=>btn.addEventListener('click',()=>{if(result)PCGDEngine.exportGroup(result,btn.dataset.exportGroup,sourceBase())}));
  ...
  function setExportState(enabled){exportBtn.disabled=!enabled;document.querySelectorAll('[data-export-group]').forEach(b=>b.disabled=!enabled);}

  async function run(){
    if(!selectedFiles.length)return;analyze.disabled=true;setExportState(false);setStatus('Đang đọc phiếu điều tra, chuẩn hóa dữ liệu và sinh các biểu PCGD–XMC…','info');
    try{
      const items=[];
      ...
      renderAll();setExportState(true);
      setStatus(`Hoàn tất: ${fmt(result.summary.total)} đối tượng · ${fmt(result.summary.villages)} thôn/xóm · ${fmt(result.summary.issues)} vấn đề cần rà soát. Có thể xuất riêng từng nhóm biểu hoặc xuất toàn bộ hồ sơ.`,'success');
  ...
  function renderAll(){renderKpis();renderAge();renderChecks();renderErrors();renderData();renderGroups();$('errorCountPill').textContent=fmt(result.summary.issues)}
  ...
  function renderGroups(){
    const sets={mn:['MN-1TE','MN-2','MN-CSVC','MN-ĐN'],th:['TH-1TE','TH-2','TH-CSVC','TH-DN'],thcs:['THCS-1TTN','THCS-2.1','THCS-2.2','THCS-CSVC','THCS-DN'],xmc:['CMC-1','CMC-2','CMC-3','CMC-4']};
    Object.entries(sets).forEach(([k,names])=>{const el=$(`group-${k}`);if(el)el.innerHTML=names.map(name=>`<li><strong>${name}</strong><span>${/CSVC|ĐN|DN/.test(name)?'Cần bổ sung dữ liệu trường':'Tự động từ điều tra'}</span></li>`).join('');});
    $('sourceList').innerHTML=result.sources.map(x=>`<li><strong>${esc(x.name)}</strong>: ${fmt(x.rows)} đối tượng${x.fileYear?` · năm ${x.fileYear}`:''}</li>`).join('');
  }
