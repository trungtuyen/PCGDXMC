(function(global){
  ...
  const inSet=(v,arr)=>arr.includes(s(v));
  ...
  function exactMap(rows,keyIndex,valueIndex){const m=new Map();rows.forEach(r=>{const k=s(r[keyIndex]);if(k!=='')m.set(k,s(r[valueIndex]));});return m;}
  function lookup(map,key){return map.get(s(key))||''}
  ...
  function fallbackLevel(rawGrade){const t=s(rawGrade).toUpperCase();if(!t)return'';if(/^SV/.test(t))return'SV';if(/^MN|^MG|^NT/.test(t))return'MN';const g=n(rawGrade);if(g>=1&&g<=5)return'TH';if(g>=6&&g<=9)return'THCS';if(g>=10&&g<=12)return'THPT';return'';}
  ...
  function computeOne(row,rowNumber,year,refs){
    ...
    const bh=lookup(refs.gradeLevel,rawGrade)||fallbackLevel(rawGrade),bf=lookup(refs.ageLearning,age)||fallbackAgeLearning(Number(age)),bg=lookup(refs.birthYearGroup,birthYear)||fallbackAgeGroup(Number(age));
    const ba=lookup(refs.schoolSystem,vSchool),bb=(['MN','TH','THCS','THPT'].includes(bh)&&!lookup(refs.schoolValid,vSchool))?'Lỗi':'';
    const bi=(bg==='15-18T'&&inSet(row[COL.W],['THCS','THPT']))?'TNC2':'';
    const br=[COL.AJ,COL.AK,COL.AL,COL.AM,COL.AN,COL.AO,COL.AP,COL.AQ].some(i=>nonblank(row[i]))?1:0;
    let bs='';if(s(row[COL.Y])===prev&&['MN','TH','THCS','THPT'].includes(s(row[COL.W])))bs=s(row[COL.W]);
    let bj='';if(bs==='THPT')bj=ba===''?'PT':ba==='TX'?'TX':ba==='NN'?'NN':'';else if(bg==='15-18T'&&bi==='TNC2'&&vSchool&&n(rawGrade)>9)bj=ba===''?'PT':ba==='TX'?'TX':ba==='NN'?'GDNN':'';
    const bk=lookup(refs.localSchool,vSchool),bl=((n(row[COL.AC])===9&&s(row[COL.AD])===prev)||(n(row[COL.AE])===9&&s(row[COL.AF])===prev))?'L9C':'';
    let bm='';if(age!==''&&Number(age)>=15){if(age<26)bm=25;else if(age<36)bm=35;else if(age<61)bm=60;}
    let bn='';if(n(row[COL.AI])>0&&br!==1)bn='MC';else if(br===1||bm==='')bn='';else if(inSet(row[COL.W],['TH','THCS','THPT']))bn='';else if(['thcn','cao đẳng','đại học','thạc sĩ','tiến sĩ'].includes(norm(row[COL.Z])))bn='';else if(n(row[COL.AC])>=5||n(row[COL.AE])>=6||n(row[COL.AH])>=5)bn='';else bn='MC';
    let bp='';if(n(row[COL.AI])===1)bp='';else if(n(row[COL.AI])===2)bp='MC2';else if(bn==='MC'&&(n(row[COL.AC])>=3||n(row[COL.AG])>3||n(row[COL.AE])>3||n(row[COL.AH])>=3))bp='MC2';
    ...
    return {rowNumber,name:fullname(row),village:s(row[COL.A]),ticket:s(row[COL.O]),birthYear,age,female:s(row[COL.H]),rawGrade:s(rawGrade),className:s(row[COL.S]),school:vSchool,graduated:s(row[COL.W]),graduationYear:s(row[COL.Y]),ba,bb,bc:age,bd:s(row[COL.A]),be:s(row[COL.O]),bf,bg,bh,bi,bj,bk,bl,bm,bn,bo,bp,bq,br,bs,bt,bu,raw:row};
  }

  function validate(records,year){
    const issues=[],seen=new Map();const add=(r,severity,message,suggestion)=>issues.push({severity,rowNumber:r.rowNumber,name:r.name||'(chưa có họ tên)',village:r.village||'',message,suggestion});
    records.forEach(r=>{
      ...
      const key=[norm(r.name),r.birthYear,s(r.raw[COL.E]),s(r.raw[COL.F]),norm(r.village)].join('|');if(r.name&&r.birthYear){if(seen.has(key))add(r,'warning',`Có khả năng trùng đối tượng với dòng ${seen.get(key)}.`,'Đối chiếu họ tên, ngày sinh và thôn/xóm.');else seen.set(key,r.rowNumber);}
    });return issues;
  }
  function summarize(records,issues,year){
    const count=p=>records.filter(p).length,households=new Set(records.map(r=>r.ticket).filter(Boolean)).size;
    ...
    const aged1518=records.filter(r=>r.bg==='15-18T'),tn1518=aged1518.filter(r=>r.bi==='TNC2').length,rate1518=aged1518.length?tn1518/aged1518.length*100:0;
    const age1560=records.filter(r=>r.age>=15&&r.age<=60&&r.br!==1),mc1560=age1560.filter(r=>r.bn==='MC').length,notMcRate=age1560.length?(age1560.length-mc1560)/age1560.length*100:0;
  ...
  function analyzeWorkbook(wb,requestedYear){
    ...
    const rows=rowsOf(wb.Sheets.DATA,'A1:BU20050'),fileYear=n(rows?.[1]?.[COL.A]),year=(fileYear>=2000&&fileYear<=2100)?fileYear:Number(requestedYear);if(!year)throw new Error('Không xác định được năm điều tra.');
    const refs=buildReferenceMaps(wb),records=[];for(let i=4;i<rows.length;i++){const row=rows[i]||[];const hasData=[COL.A,COL.B,COL.C,COL.D,COL.G,COL.O].some(c=>nonblank(row[c]));if(!hasData)continue;records.push(computeOne(row,i+1,year,refs));}
    const issues=validate(records,year),summary=summarize(records,issues,year);return{year,records,issues,summary};
  }
  function exportResult(result,sourceName){
    const wb=XLSX.utils.book_new(),s=result.summary;
    ...
    const ws1=XLSX.utils.aoa_to_sheet(summaryRows);ws1['!cols']=[{wch:44},{wch:22}];XLSX.utils.book_append_sheet(wb,ws1,'TongHop');
    const issueRows=[['Mức độ','Dòng Excel','Họ tên','Thôn/xóm','Nội dung','Gợi ý'],...result.issues.map(i=>[i.severity,i.rowNumber,i.name,i.village,i.message,i.suggestion])];const ws2=XLSX.utils.aoa_to_sheet(issueRows);ws2['!cols']=[{wch:10},{wch:12},{wch:30},{wch:24},{wch:55},{wch:55}];XLSX.utils.book_append_sheet(wb,ws2,'SoatLoi');
    const dataRows=[['Dòng','Họ tên','Thôn/xóm','Số phiếu','Năm sinh','Tuổi','Khối hiện tại','Lứa tuổi học','Nhóm tuổi','TNC2','Hệ THPT','Tại chỗ','L9C','Nhóm XMC','Mù chữ','MC1','MC2','Bỏ học','Khuyết tật','TN năm trước','Lưu ban','2 buổi','Lỗi mã trường'],...result.records.map(r=>[r.rowNumber,r.name,r.village,r.ticket,r.birthYear,r.age,r.bh,r.bf,r.bg,r.bi,r.bj,r.bk,r.bl,r.bm,r.bn,r.bo,r.bp,r.bq,r.br,r.bs,r.bt,r.bu,r.bb])];const ws3=XLSX.utils.aoa_to_sheet(dataRows);ws3['!cols']=dataRows[0].map((_,i)=>({wch:i===1?30:i===2?24:14}));XLSX.utils.book_append_sheet(wb,ws3,'DuLieuTinh');
