(function(global){
  ...
  function ensureStyle(){
    ...
    const st=document.createElement('style');st.id='pcgdThreeLevelStyle';st.textContent=`
      #pcgdLoginPortal{position:fixed;inset:0;z-index:10000;display:none;place-items:center;overflow:auto;padding:max(16px,env(safe-area-inset-top)) max(16px,env(safe-area-inset-right)) max(16px,env(safe-area-inset-bottom)) max(16px,env(safe-area-inset-left));background:linear-gradient(135deg,#e9f3f7 0%,#f6fbf8 48%,#e8f2f8 100%);font-family:Arial,sans-serif;-webkit-overflow-scrolling:touch}
      ...
      .tl-login-body{padding:22px 26px 25px}.tl-field{display:flex;flex-direction:column;gap:6px;margin-bottom:13px}.tl-field label{font-size:12px;font-weight:800;color:#314e5d}.tl-field input,.tl-field select{height:46px;border:1px solid #b9cbd5;border-radius:9px;padding:0 12px;background:#fff;font-size:16px;color:#173744;outline:none}.tl-field input:focus,.tl-field select:focus{border-color:#1874a3;box-shadow:0 0 0 3px rgba(24,116,163,.12)}
      #pcgdPortalLoginBtn{width:100%;min-height:46px;border:0;border-radius:9px;background:#126f98;color:#fff;font-size:15px;font-weight:800;cursor:pointer;touch-action:manipulation}#pcgdPortalLoginBtn:disabled{opacity:.65;cursor:wait}.tl-login-msg{min-height:18px;margin-top:10px;font-size:12px;color:#667c87}.tl-login-msg[data-kind="error"]{color:#b42318}.tl-login-note{margin-top:13px;padding:9px 10px;border-radius:8px;background:#f1f7fa;color:#58717e;font-size:11px;line-height:1.45}.tl-net{display:flex;align-items:center;gap:6px;margin-top:8px;font-size:11px;font-weight:700}.tl-net-dot{width:8px;height:8px;border-radius:50%;background:#1f9d55}.tl-net[data-online="false"]{color:#b42318}.tl-net[data-online="false"] .tl-net-dot{background:#d92d20}
      #pcgdRoleBar{display:none;max-width:1180px;margin:12px auto 0;padding:0 18px}.tl-role-inner{display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid #cbdbe2;background:#fff;border-radius:11px;padding:10px 12px;box-shadow:0 3px 12px rgba(35,73,89,.07)}.tl-role-info strong{display:block;font-size:14px;color:#183f51}.tl-role-info span{display:block;margin-top:3px;font-size:11px;color:#667e89}.tl-role-user{display:flex;align-items:center;gap:9px}.tl-role-user div{text-align:right}.tl-role-user b{display:block;font-size:12px}.tl-role-user small{font-size:10px;color:#6a7f89}.tl-role-user button{min-height:36px;padding:0 10px;touch-action:manipulation}
      ...
      @media(max-width:600px){#pcgdLoginPortal{place-items:start center}.tl-login-card{border-radius:12px;margin:auto 0}.tl-login-head,.tl-login-body{padding-left:18px;padding-right:18px}.tl-role-inner{align-items:flex-start;flex-direction:column}.tl-role-user{width:100%;justify-content:space-between}.tl-role-user div{text-align:left}#pcgdRoleBar{padding:0 10px}.tl-role-user button{min-height:42px}}
  ...
  function ensurePortal(){
    ...
    const portal=document.createElement('section');portal.id='pcgdLoginPortal';portal.innerHTML=`
      <div class="tl-login-card" role="dialog" aria-modal="true" aria-labelledby="pcgdPortalTitle">
        ...
        <div class="tl-login-body">
          ...
          <div class="tl-field"><label for="pcgdPortalUser">Tên đăng nhập</label><input id="pcgdPortalUser" autocomplete="username" autocapitalize="none" spellcheck="false" placeholder="Tên đăng nhập"></div>
          ...
          <div id="pcgdNetworkState" class="tl-net" data-online="true"><span class="tl-net-dot"></span><span>Kết nối Internet sẵn sàng · Wi‑Fi/4G/5G</span></div>
    ...
    const doLogin=async()=>{
      ...
      if(!online()){msg.textContent='Thiết bị đang mất Internet. Hãy bật 4G/5G hoặc Wi‑Fi rồi thử lại.';msg.dataset.kind='error';return}
  ...
  function filterAdminRows(level,u){
    document.querySelectorAll('#pauUsers tr').forEach(row=>{
      ...
      if(u?.role==='super_admin'){row.style.display='';return}
      row.style.display=level==='ministry'?(text==='Quản trị tỉnh'?'':'none'):level==='province'?(text==='Quản trị xã'?'':'none'):'';
    });
  }

  function renderNetwork(){const el=$('pcgdNetworkState');if(!el)return;el.dataset.online=String(online());el.querySelector('span:last-child').textContent=online()?'Kết nối Internet sẵn sàng · Wi‑Fi/4G/5G':'Đang ngoại tuyến · bật 4G/5G hoặc Wi‑Fi'}

  function render(){
    const a=auth();if(!a)return;
    const u=user(),portal=$('pcgdLoginPortal'),bar=$('pcgdRoleBar');renderNetwork();
    if(!u){
    ...
    if(bar){bar.style.display='block';bar.innerHTML=`<div class="tl-role-inner"><div class="tl-role-info"><strong>${esc(meta.title)}</strong><span>${esc(meta.desc)}</span></div><div class="tl-role-user"><div><b>${esc(u.displayName||u.username)}</b><small>${esc(meta.label)} · ${esc(u.username)} · ${online()?'Online':'Offline'}</small></div><button id="pcgdRoleLogout" class="secondary" type="button">Đăng xuất</button></div></div>`;$('pcgdRoleLogout').onclick=()=>a.logout()}
    setTimeout(()=>{lockAdminHierarchy(u,level);filterAdminRows(level,u);global.PCGDNational?.renderAggregate?.()},80);
  }
  ...
  function init(){
    ...
    global.addEventListener('pcgd-network-changed',()=>setTimeout(render,0));
    global.addEventListener('online',()=>setTimeout(render,0));
    global.addEventListener('offline',()=>setTimeout(render,0));
    const mo=new MutationObserver(()=>{const u=user();if(u){const level=levelOf(u);lockAdminHierarchy(u,level);filterAdminRows(level,u)}});mo.observe(document.body,{childList:true,subtree:true});
    render();
  }
