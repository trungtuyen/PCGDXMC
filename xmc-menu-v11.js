(function(){
  ...
  dropdown.innerHTML=`
    ...
    <div class="xmc-dd-sep"></div>
    <div class="xmc-dd-head">Biểu tổng hợp theo nhóm tuổi</div>
    ...
    <div class="xmc-dd-sep"></div>
    <div class="xmc-dd-head">Danh sách đối tượng</div>
    <button type="button" data-xmc-target="[data-mc-level='MC1']">Người mù chữ mức độ 1</button>
    <button type="button" data-xmc-target="[data-mc-level='MC2']">Người mù chữ mức độ 2</button>
  `;
  document.body.appendChild(dropdown);

  function position(){
    const r=tab.getBoundingClientRect();
    dropdown.style.left=`${Math.max(4,Math.min(r.left,window.innerWidth-dropdown.offsetWidth-6))}px`;
  ...
  dropdown.querySelectorAll('[data-xmc-target]').forEach(item=>item.addEventListener('click',()=>{
    const selector=item.dataset.xmcTarget;
    const target=sourceMenu.querySelector(selector);
    close();
    activateXmcPage();
    if(target){
      target.click();
      document.getElementById('xmc')?.scrollIntoView({behavior:'smooth',block:'start'});
    }
  }));

  document.addEventListener('click',ev=>{
    if(dropdown.style.display==='block'&&!dropdown.contains(ev.target)&&ev.target!==tab)close();
  });
